...
app.put('/products/:id', (req, res) => {
  const productId = req.params.id;
  const { name, price, description, category, image, imagge_2, image_3, cpu, ram, sd, manhinh, card } = req.body;

  const query = 'UPDATE product SET name = ?, price = ?, description = ?, category = ?, image = ?, imagge_2 = ?, image_3 = ?, cpu = ?, ram = ?, sd = ?, manhinh = ?, card = ? WHERE id = ?';
  ...
  db.execute(query, [name, price, description, category, image, imagge_2, image_3, cpu, ram, sd, manhinh, card, productId], (err, results) => {
    ...
    res.status(200).json({ message: 'Sản phẩm đã được sửa' });
  });
});
